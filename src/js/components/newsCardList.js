...
import { isValidLink } from './validate';
import { getCookie } from '../utils/cookies';
import { ErrorValidationMessages, ErrorMessages } from '../resources/messages';

export { NewsCardList };

const sanitizeSpanHTML = (str) => {
...
class NewsCardList {
  ...
  _bookmarkClick(event) {
    ...
    if (this.mainApi.isLogedUser()) {
      ...
      const searchInput = this.searchInput ? searchInput.value : '';
      const article = {
        keyword: clearStr(searchInput, 3, 30),
  ...
  _searchButtonClick(event) {
    event.preventDefault();
    const searchInput = this.searchInput ? this.searchInput.value.trim() : '';
    if (searchInput === '') {
    ...
    this.newsApi.getNews({ newsQuery: searchInput.value, dateFrom: getNewsDate(new Date(), 0), dateTo: getNewsDate(new Date(), -NEWS_PERIOD) })
      .then((res) => {
        ...
        return Promise.reject({ message: res.statusText, res });
      })
      .then((result) => {
        if (result.status === 'ok') {
  ...
  renderSavedItems() {
    ...
    this.mainApi.getArticles()
      ...
      .then((result) => {
        const res = result.data.map((element) => this.getNewsDataFormat(element));
